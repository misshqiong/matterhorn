...
def test_human_rank_beats_later_recorded_model_and_conflict_is_counted() -> None:
    human = _assertion(
        "human", datetime(2026, 1, 1, 9, tzinfo=timezone.utc), "human"
    )
    model = _assertion(
        "model", datetime(2026, 1, 1, 12, tzinfo=timezone.utc), "model"
    )
...
def test_same_value_at_same_or_later_instant_does_not_split() -> None:
    first = _assertion("same", datetime(2026, 1, 1, 9, tzinfo=timezone.utc))
    second = first.model_copy(
        update={
            "assertion_id": "f" * 64,
            "valid_from": datetime(2026, 1, 2, tzinfo=timezone.utc),
            "recorded_at": datetime(2026, 1, 2, 9, tzinfo=timezone.utc),
        }
    )
